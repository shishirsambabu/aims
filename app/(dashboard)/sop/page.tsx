  ...
  return (
    <div>
      ...
      <div className="space-y-6 p-6">
        <section className="overflow-hidden rounded-lg border border-border bg-sidebar text-white shadow-card">
          <div className="grid gap-0 lg:grid-cols-[0.9fr_1.1fr]">
            ...
            <div className="grid gap-3 border-t border-white/10 bg-white/[0.06] p-5 md:grid-cols-2 lg:border-l lg:border-t-0">
              {modules.map((module) => (
                <div key={module} className="rounded-lg border border-white/15 bg-white/10 p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-white/60">{module}</p>
                  <p className="mt-2 font-financial text-3xl font-bold">
                    {SOP_PLAYBOOKS.filter((playbook) => playbook.module === module).length}
                  </p>
                  <p className="mt-1 text-xs text-white/60">linked playbooks</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid gap-5 xl:grid-cols-2">
          {SOP_PLAYBOOKS.map((playbook) => {
            ...
            return (
              <Card key={playbook.id} className="overflow-hidden rounded-lg shadow-card">
                <div
                  className={cn(
                    "h-2",
                ...
                <CardContent className="space-y-5 p-6">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="flex items-start gap-3">
                      <div className="rounded-lg border border-border bg-primary/10 p-3 text-primary">
                  ...
                  <div>
                    ...
                    <div className="relative mt-3 space-y-3">
                      ...
                      {playbook.steps.map((step, index) => (
                        <div key={step.title} className="relative rounded-lg border border-border bg-surface-alt/40 px-4 py-3">
                          <div className="flex items-start gap-3">
